...
#Default SE in 2D so we apply our custom SE
...
class ConvModule(nn.Module):
    def __init__(self, nchan, elu, se):
        ...
        if se:
            self.se1 = SqueezeExcitation(input_channels=nchan, squeeze_channels=nchan)  
...
class VNetRec(nn.Module):
    def __init__(self, elu=False, se=True, input_ch=1, split_ch=16):
        super(VNetRec, self).__init__()
